...
class gSpan(object):
    ...
    def __init__(self,
                 ...
                 is_undirected=True,
                 node_label_name='node_name',
                 edge_label_name='op_name',
                 verbose=False,
                 visualize=False,
                 where=False,
                 save_path=None):
        ...
        database_file:
            if 'plain' or 'networkx_pickle', expecting a path string leading to a valid database file
            if 'networkx', expecting a list/iterable of networkx graphs.
        """
        assert file_type in ['networkx', 'networkx_pickle', 'plain'], f'unexpected file_type keyword {file_type}'
        ...
        self._subgraphs_nx = []
        self._graph_hashes = []     # store the weisfeiler-lehman hashes of the sub-graphs.
    ...
    def _read_graphs(self):
        self.graphs = dict()
        with codecs.open(self._database_file, 'r', 'utf-8') as f:
    ...
    @record_timestamp
    def _read_graphs_from_networkx(self):
        """Similar to above but read graphs from a pickle of list of networkx graphs instead"""
        self.graphs = dict()
        if self.file_type == 'networkx_pickle':
            data = pickle.load(open(self._database_file, 'rb'))
            data = [d['graph'] for d in data]
        elif self.file_type == 'networkx':
            data = self._database_file
        # select the networkx graphs only.
        for graph_cnt, networkx_graph in enumerate(data):
            tgraph = Graph(graph_cnt,
                           is_undirected=self._is_undirected and not isinstance(networkx_graph, nx.DiGraph),
                           eid_auto_increment=True)
            for node, data in networkx_graph.nodes(data=True):
                tgraph.add_vertex(node, data[self.node_label_name])
            for ei, eo, data in networkx_graph.edges(data=True):
                tgraph.add_edge(AUTO_EDGE_ID, ei, eo, data[self.edge_label_name])
            self.graphs[graph_cnt] = tgraph
        return self
    ...
    def save(self):
        ...
        file_to_save = {
            ...
            'subgraphs_nx': self._subgraphs_nx,
            'subgraphs_hash': self._graph_hashes,
            'incidence': self._support_idx
        }
        pickle.dump(file_to_save, open(self.save_path, 'wb'))
        print(f'Info saved to {self.save_path}')
    ...
    def run(self):
        ...
        if self.file_type == 'plain':
            ...
            self._read_graphs_from_networkx()
    ...
    def _report(self, projected):
        ...
        g_nx = g.to_networkx()
        self._subgraphs_nx.append(g_nx)
        g_hash = nx.algorithms.graph_hashing.weisfeiler_lehman_graph_hash(g_nx,
                                                                          node_attr='label' if self.node_label_name is not None else None)
        self._graph_hashes.append(g_hash)
        # also store the ids where this subgraph has appeared.
        self._support_idx.append(set([p.gid for p in projected]))
