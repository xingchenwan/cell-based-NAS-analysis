from .kernels.weisfeiler_lehman import WeisfeilerLehman
from .kernels.continuous_wl import ContinuousWeisfeilerLehman
from .kernels.vertex_histogram import VertexHistogram
import numpy as np
...
class WeisfeilerLehmanExtractor:
    ...
    def fit(self, g_list: list):
        ...
        if self.mode == 'categorical':
            ...
            train_graphs = graph_from_networkx(g_list, self.node_attr)
        else:
    ...
    def update(self, g_list: list):
        ...
        if self.mode == 'categorical':
            if self.base_kernels is None:
                ...
                eval_graphs = graph_from_networkx(g_list, self.node_attr)
            ...
            if self.wl is None or self.wl.X is None:
                ...
                feat_vector = self.wl.parse_input(g_list)
    ...
    def transform(self, h_list: list):
        ...
        if self.mode == 'categorical':
            ...
            eval_graphs = graph_from_networkx(h_list, self.node_attr)
            ...
            feature_vector = self.wl.parse_input(h_list)
