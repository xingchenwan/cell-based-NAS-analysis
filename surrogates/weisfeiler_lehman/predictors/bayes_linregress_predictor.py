...
from surrogates.weisfeiler_lehman.utils import to_unit_cube, to_unit_normal, from_unit_normal
from surrogates.weisfeiler_lehman.wl_extractor import WeisfeilerLehmanExtractor
from sklearn import linear_model
import numpy as np


class BayesianLinearRegression(BaseWLPredictor):

    def __init__(self, h=1, max_step=1000,
                 verbose=False,
                 model_choice='ard',
        ...
        assert model_choice in ['ard', 'blr', 'ols']
    ...
    def predict(self, x_eval: list, include_noise_variance=False, **kwargs) -> (torch.Tensor, torch.Tensor):
        ...
        if self.model in ['ard', 'blr']:
            mean, std = self.model.predict(x_feat_vector, return_std=True)
        else:
            mean = self.model.predict(x_feat_vector)
            std = np.nan
        variance = std ** 2
